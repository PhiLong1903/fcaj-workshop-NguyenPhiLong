...
flowchart LR
  ...
  subgraph AWS_Cloud["AWS Cloud"]
    ...
    %% Xử lý bất đồng bộ & Tác vụ AI
    subgraph Async_Tasks["8b. Async Orchestration & AI Tasks"]
    ...
    %% Các dịch vụ Data & AI được AWS quản lý
  ...
  %% --- CÁC LUỒNG TƯƠNG TÁC (FLOWS) ---

  %% Tương tác từ phía người dùng
  User -- "1. Fetch UI" --> Amplify
  User -- "2. Login, lấy token" --> Cognito
  User -- "3. Request + JWT" --> WAF

  %% Luồng trong Edge & Auth Layer
  ...
  %% Luồng gọi API đồng bộ (Sync Call)
  ...
  %% Luồng gọi API bất đồng bộ (Async Execution)
  ...
  %% Tương tác của các tác vụ AI với dịch vụ Managed Services
